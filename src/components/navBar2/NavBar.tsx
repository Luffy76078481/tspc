...
class NavBar extends BaseClass {
    ...
    renderUserInfo() {
        return (
            <div className="userInfo">
                <Link to="/myCenter/news" className="news">
                    {this.props.unred > 0 && <div className="newsCont">{this.props.unred}</div>}
                    <i className={`icon iconfont iconxiaoxi ${this.props.unred>0 && "iconAni"}`}></i>
                </Link>
                <div className="xian"></div>
                <div className="rightTop">
                    <img className="vip_logo" src={require('./images/vip.jpg')} alt=""/>
                    <Link to="/myCenter/userInfo" className="rightTopName">
                ...
                <div className="rightBottom">
                    <Link to="/myCenter/deposit" className="money">钱包:<span className="headMoney">{this.props.user.amount}</span>元</Link>
                    <span className="loginOut" onClick={this.onLogout.bind(this)}>退出登录</span>
                    <div className="headPimg">
                        <img src={require("./images/head.png")} alt="" />
                    </div>
                </div>
                {/* <span>欢迎您, </span>
                <Link to="/myCenter">{this.props.user.UserName}&nbsp;&nbsp;｜&nbsp;&nbsp;</Link>
                ...
                <Link to="/records_message">消息</Link>
                <span className="loginOut" onClick={this.onLogout.bind(this)}>退出登录</span> */}
            </div>
        )
    }
    ...
    renderNav() {
        ...
        for (let i = 0; i < mainNav.length; i++) {
            let mainNavData:{Title:string,GotoUrl:string,Tag:string} = mainNav[i];
            // 手机APP
            if( (mainNavData.Tag==="mobile"||mainNavData.Title.includes('手机')) && !mainNavData.GotoUrl){
                mainNavDom.push(
                    <a key={i} href={this.props.remoteSysConfs.channel_push_url} rel="noopener noreferrer" target="_blank">
                        <span>{mainNavData.Title}</span>
                        <i className="icon iconfont iconjiantou"></i>
                    </a>
                )
            }
            // else if(mainNavData.GotoUrl.includes('www')){
            //     mainNavDom.push(
            ...
            // }
                ...
                mainNavDom.push(
                    <a key={i} onClick={() => { this.serversOpen() }}>
                        <span>{mainNavData.Title}</span>
                        <i className="icon iconfont iconjiantou"></i>
                    </a>
                )
            ...
            if ((mainNav[i].Tag === "casino" || mainNav[i].Tag === "fish" || mainNav[i].Tag === "bingo" || mainNav[i].Tag === "chess") && mainNav[i].Games) {
                        ...
                        <div key={i} className="secNavBarBox" style={{ display: this.state.activeIndex === i ? "flex" : "none" }}>
                            {
                                mainNav[i].Games.map((data: any, index: number) => {                          
                                    return (
                                        <div key={index} className="secNavBar" style={{ flexGrow: mainNav[i].Data.length<6?0:1 }} onClick={this.playGame.bind(this, data)}>
                ...
                if (mainNav[i].Data.length < 7) {
                    navBarDom.push(//循环二级导航
                        <div key={i} className="secNavBarBox" style={{ display: this.state.activeIndex === i ? "flex" : "none" }}>
                             {
                                 mainNav[i].Tag === "sport" ? <div className="hint"><img src={require("./images/fanshui.png")} alt=""/></div> : null
                             }
                            {
                                
                                mainNav[i].Data.map((data: any, index: number) => {
                                    return (
                                        <div key={index} className="secNavBar" style={{ flexGrow: mainNav[i].Data.length<6?0:1 }} onClick={this.goGamePage.bind(this, data)}>
